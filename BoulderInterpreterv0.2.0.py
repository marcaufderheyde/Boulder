...
# Welcome to the Boulder programming language interpreter v0.2.0
...
#
# Fixes/Bugs: 
# ==>   As of version v0.2.0 the Boulder programming language supports function definitions
#
# ==>   The function implementation is slightly buggy and might have unexpected side effects
#       when used, please report any findings on the GitHub. For instance, not sure if multiple
#       parameters are working for function definitions or any functions actually!
#
# Roadmap:
# ==>   Look at other esoteric languages and steal ideas
...
# Dictionary used for converting the Boulder functions when defining new functions
    ...
    # Otherwise continue and try to interpret
        ...
        try:    
            ...
            for line in program_lines:
                ...
                # Let's check first if we've got a function definition
                if("HOLD" == line.split(" ")[0]):
                    # Skip the first element as it's just going to be HOLD
                    function_name = line.split(" ")[1].split("(")[0]
                    function_args = line.split("(")[1].split(")")[0].split(",")
                    function_code = line.split("=>")[1].strip(" ").split("SLOPER")
                    function_code_in_python = []
                    
                    # We need to convert the Boulder function calls to python calls
                    for code_snip in function_code:
                        func_name_to_convert = code_snip.strip().split("(")[0]
                        python_equivalent = documentation_strings[func_name_to_convert].strip()
                        func_params_to_carry = "(" + code_snip.strip().split("(")[1]
                        if(func_params_to_carry[-1] == "f"):
                            func_params_to_carry = "(float" + func_params_to_carry[:-1] + ")"
                        elif(func_params_to_carry[-1] == "i"):
                            func_params_to_carry = "(int" + func_params_to_carry[:-1] + ")"
                        converted_function_call = python_equivalent + func_params_to_carry
                        function_code_in_python.append(converted_function_call)
                    function_code = '\n\t'.join(function_code_in_python)
                                    
                    # Create the function dynamically
                    function_definition = f"def {function_name}({', '.join(function_args)}):\n\t{function_code}"
                    
                    exec(function_definition, globals())
                    documentation_added_functions[function_name] = function_definition
                ...
                while len(components) > 0 and (index - 1) >= 0:
                    ...
                    match param_type:
                        ...
                        case "s":
                            if(components[index-1] in documentation_added_functions):
                                print(components[index-1])
                                eval(str(components[index-1] + "(" + (components[index]) + ")"))
                                
                            # If we can find the function name in our documentation dictionary, it's a valid line
                            elif(components[index-1] in documentation):
                                lines_recognized += 1
