...
# Welcome to the Boulder programming language interpreter v0.0.2
    ...
    # Otherwise continue and try to interpret
        ...
        try:    
            ...
            for line in program_lines:
                ...
                # This might need to be changed to accomodate chaining calls
                # So currently this split and strip is actually working as we want it,
                # can we use that format recursively or something?
                components = line.strip(")").split("(")
                
                # This works for getting all our function calls and the parameter but how to apply
                # to the below implementation?
                if(len(components) > 2):
                    multi_functional = components.copy()
                    index = len(multi_functional) - 1
                    while len(multi_functional) > 0:
                        del multi_functional[index]
                        index -= 1
                ...
                if(components[0] in printer_funcs or components[0] in printer_funcs_no_input or param_type not in ['s', 'i', 'f']):
                    param_type = 's'
                ...
                match param_type:
                    ...
                    case "s":
                        ...
                        if(components[0] in documentation):
                            ...
                            format_supported = components[1].split(")")[0].strip("\"") !=''

                            # Just print directly if supported with no input
                            if(components[0] in printer_funcs and format_supported):
                                documentation[components[0]](components[1].split(")")[0].strip("\""))
                            
                            # Just print directly if supported with no input
                            elif(components[0] in printer_funcs_no_input):
                                documentation[components[0]]()
                                
                            # List functions require some tweaking for the params as they're coming as strings    
                            elif(components[0] in list_funcs and format_supported):
                            ...
                            # Otherwise provide a representation of the computation
                                ...
                                result = documentation[components[0]](components[1].split(")")[0].strip("\""))
                                print("Result of: " + line + " is: " + str(result) + " which is of type: " + str(type(result)))
                            
                            else:
                                result = documentation[components[0]]()
                                print("Result of: " + line + " is: " + str(result) + " which is of type: " + str(type(result)))
                        else:
